 ...
 *  @section DESCRIPTION
 *
 *  Implementation of the entropy coding step.
 *
 */
...
long EntropyCoder::EntropyCode(Macroblock *mb, bool inter, int qp)
{
	...
	if (inter && DO_MC)
	{
		CodeSplitPartitions(mb->partitions);
		MotionVector pred = predictMV(mb);
		last_mb_used_bits += CodeInt(mb->mv[0].x - pred.x);
		last_mb_used_bits += CodeInt(mb->mv[0].y - pred.y);
		if (mb->partitions) {
			for (int part_num = 1; part_num < 4; ++part_num) {
				last_mb_used_bits += CodeInt(mb->mv[part_num].x - mb->mv[part_num - 1].x);
				last_mb_used_bits += CodeInt(mb->mv[part_num].y - mb->mv[part_num - 1].y);
			}
		}
	}
...
MotionVector EntropyCoder::predictMV(Macroblock *mb)
{
	...
	if (mb->mb_up != NULL)
	{
		pred.x += mb->mb_up->mv[0].x;
		pred.y += mb->mb_up->mv[0].y;
		mvs_used++;
	}
	if (mb->mb_left != NULL)
	{
		pred.x += mb->mb_left->mv[0].x;
		pred.y += mb->mb_left->mv[0].y;
		mvs_used++;
	}
...
void EntropyCoder::CodeSplitPartitions(bool partitions)
{
	bfo->WriteBits(partitions, 1);
}
